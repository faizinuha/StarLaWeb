...
if ($action == 'like' || $action == 'dislike') {
    ...
    if ($result_check_user->num_rows > 0) {
        ...
        $result_check_like = $conn->query($check_like_query);

        $check_dislike_query = "SELECT * FROM dislikes WHERE post_id = $post_id AND user_id = $current_user_id";
        $result_check_dislike = $conn->query($check_dislike_query);
        ...
        if ($result_check_like->num_rows > 0) {
            if ($action == 'like') {
                ...
                $conn->query($insert_action_query);
            }
        } elseif ($result_check_dislike->num_rows > 0) {
            if ($action == 'dislike') {
                // Hapus dislike
                $conn->query("DELETE FROM dislikes WHERE post_id = $post_id AND user_id = $current_user_id");
